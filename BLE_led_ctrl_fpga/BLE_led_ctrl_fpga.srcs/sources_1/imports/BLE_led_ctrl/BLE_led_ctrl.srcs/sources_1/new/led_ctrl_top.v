...
module led_ctrl_top (
  ...
  input stack_valid,
  output stack_ready,
  input [7:0] stack_data,

  // LED outputs
  output [2:0] led_out
);
// ------------------------------------------------------------
// Clock Divider (Slow Enable Generator)
// ------------------------------------------------------------
reg [23:0] clk_div;
reg slow_en;
...
always @(posedge clk or negedge rst_n) begin
  if (!rst_n) begin
    clk_div <= 0;
    slow_en <= 0;
  end else if (clk_div == 24'd50_000_000) begin // ~1 Hz toggle (for 100 MHz clk)
    clk_div <= 0;
    slow_en <= 1;   // 1-cycle pulse
  end else begin
    clk_div <= clk_div + 1;
    slow_en <= 0;
  end
...
// Mappingg switches to stack_data while tying Bit 13 and Bits 12:10 to Zero
  ...
  // ------------------------------------------------------------
  wire       phase_start;
  wire [23:0] phase_load;
  wire       phase_running;
  wire       phase_done;
  wire [23:0] phase_count;

  timer_down #(24) u_phase_timer (
    .clk       (clk),
    .slow_en   (slow_en),
    .rst_n     (rst_n),
    .start     (phase_start),
  ...
  // ------------------------------------------------------------
  wire       duration_start;
  wire [23:0] duration_load;
  wire       duration_running;
  wire       duration_done;
  wire [23:0] duration_count;

  timer_down #(24) u_duration_timer (
    .clk       (clk),
    .slow_en   (slow_en),
    .rst_n     (rst_n),
